...
## Hardware Access Device Names
   ...
   a) The devices can be accessed by their PCI ID as displayed by lspci
    ...
    b) When the IB driver (typically mlx5) is loaded, the devices can be accessed
...
### Others
    For other tools (mstconfig, mstreg, mstfwtrace, mstlink) see
    usage message from the -h parameter. e.g.
...
### Development
    Development is in the Github project:

       https://github.com/Mellanox/mstflint

    Development follows a roughly 3-months development cycle. At each
    cycle, the main branch (master) is mostly frozen and development
    happens at the branch master_devel.

    Therefore pull-requests should be submitted vs. branch master_devel.
