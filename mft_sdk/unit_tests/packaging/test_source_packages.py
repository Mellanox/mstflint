...
class SourcePackageSuite(object):
    ...
    def deb_invocations(self):
        ...
        want = ["{}_{}.debian.tar.xz".format(TEST_NAME, full),
                ...
                "{}_{}_{}.deb".format(TEST_NAME, full, _arch())]
    ...
    def rpm_artifacts(self):
        ...
        if not os.path.isdir(self.rpm_out):
    ...
    def deb_source_identity(self):
        """The .dsc Source/Version must match the binary package."""
        if not os.path.isdir(self.deb_out):
            return self._record("deb_source_identity", "SKIP", "no deb build")
        dsc = [f for f in os.listdir(self.deb_out) if f.endswith(".dsc")]
        if not dsc:
            return self._record("deb_source_identity", "SKIP", "no .dsc")
        with open(os.path.join(self.deb_out, dsc[0])) as f:
            body = f.read()
        m = re.search(r"^Source:\s*(\S+)\s*$", body, re.M)
        v = re.search(r"^Version:\s*(\S+)\s*$", body, re.M)
        want = "{}-{}".format(self.version, TEST_RELEASE)
        if not m or m.group(1) != TEST_NAME:
            return self._record("deb_source_identity", "FAIL",
                                "Source: {}".format(m.group(1) if m else "?"))
        if not v or v.group(1) != want:
            return self._record("deb_source_identity", "FAIL",
                                "Version: {}".format(v.group(1) if v else "?"))
        return self._record("deb_source_identity", "PASS",
                            "{} {}".format(m.group(1), want))
