...
void PCILibrary::SetPCIDomain(int& failedToSetSegmentBase)
{
    failedToSetSegmentBase = 3;
    return;
}
...
void PCILibrary::SetPCIDomain(int& failedToSetSegmentBase)
{
    ...
    failedToSetSegmentBase = 0;
    if (!pciDevices)
    {
        failedToSetSegmentBase = 1;
    ...
    for (int count = 0; count < numDevices; count++)
    {
        ...
        if (!directNicDevice.empty())
        {
            ...
            if (rc)
            {
                LOG.Error(std::string("Failed to get MPIR register for dbdf: ") + dbdf);
                mclose(mf);
                failedToSetSegmentBase = 2;
                continue;
            }
            LOG.Debug(std::string("MPIR register for dbdf: ") + dbdf + " with sdm: " + std::to_string(mpir.sdm));
            if (mpir.sdm == 1)
            {
