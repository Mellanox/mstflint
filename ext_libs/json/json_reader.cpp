...
bool OurReader::readObject(Token& token)
{
    ...
    while (readToken(tokenName))
    {
        ...
        if (!initialTokenOk)
            break;
        if (tokenName.type_ == tokenObjectEnd && (name.empty() || features_.allowTrailingCommas_)) // empty object or
                                                                                                   // trailing comma
            return true;
        name.clear();
        if (tokenName.type_ == tokenString)
...
bool OurReader::readArray(Token& token)
{
    ...
    for (;;)
    {
        skipSpaces();
        if (current_ != end_ && *current_ == ']' &&
            (index == 0 || (features_.allowTrailingCommas_ && !features_.allowDroppedNullPlaceholders_))) // empty array
                                                                                                          // or trailing
                                                                                                          // comma
        {
            Token endArray;
